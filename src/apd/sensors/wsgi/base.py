...
def set_up_config(
    environ=None, to_configure=None,
):
    if environ is None:
    ...
    data_file = os.path.join(os.getcwd(), "sensor_data.sqlite")
    environ["SQLALCHEMY_DATABASE_URI"] = environ.get(
        "APD_SENSORS_DB_URI", f"sqlite:///{data_file}"
    )
